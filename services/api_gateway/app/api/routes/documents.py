...
from app.services.ingestion import submit_document
...
async def upload_document(
    ...
    submit_document(background_tasks, document_id, str(target_path))
    return document


@router.post("/ingest-sample", response_model=List[DocumentRead])
def ingest_sample_documents(tenant: CurrentTenant, db: Session = Depends(get_db)) -> list[Document]:
    ...
    for path in sorted(sample_dir.glob("*")):
        ...
        db.commit()
        db.refresh(document)
        # Sample ingestion is synchronous so the demo is immediately searchable.
...
def list_documents(tenant: CurrentTenant, db: Session = Depends(get_db)) -> list[Document]:
    return (
        db.query(Document)
...
def get_document(document_id: str, tenant: CurrentTenant, db: Session = Depends(get_db)) -> DocumentDetailRead:
    document = (
        db.query(Document)
        .filter(Document.id == document_id, Document.tenant_id == tenant)
        .first()
    )
    if not document:
