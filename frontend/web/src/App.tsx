          ...
          {searchResult && (
            <div className="search-results">
              ...
              {searchResult.strategy && (
                <p className="muted">
                  retrieval: {searchResult.strategy.embedding_backend} · {searchResult.strategy.vector_backend}
                  {searchResult.strategy.reranked ? " · reranked" : ""} · {searchResult.strategy.dense_candidates}d/
                  {searchResult.strategy.sparse_candidates}s · {searchResult.strategy.latency_ms} ms
                </p>
              )}
                ...
                {searchResult.evidence.map((item) => (
                  <div className="evidence-card" key={`${item.document_id}-${item.score}`}>
                    ...
                    <p className="muted">
                      ...
                      {typeof item.dense_score === "number" ? ` · dense ${item.dense_score.toFixed(3)}` : ""}
                      {typeof item.sparse_score === "number" ? ` · bm25 ${item.sparse_score.toFixed(2)}` : ""}
                    </p>
                    <p>{item.text}</p>
                  </div>
                ))}
